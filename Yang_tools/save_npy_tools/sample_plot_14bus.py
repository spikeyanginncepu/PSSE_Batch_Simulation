...
sample_id = 500  # 注意：这是初始样本 id，修改下面的range没用 ！！！
...
for i in range(0, 1):
	for j in range(12):
		...
		plt.xlabel("Time")
		plt.ylabel("Angle")
		# plt.title("Rotor Angle Curve -- "+'Bus'+str(i+1)+' Fault')    # 母线故障用
		plt.title("Rotor Angle Curve -- " + 'Branch Fault -- Position' + str(i + 1))  # 支路故障用
		plt.legend()
		axes = plt.gca()
		axes.set_xlim([0, 10])
		# _adjust() 语句放在句末，防止还没出现列子图，导致 hspace 参数报错
		plt.subplots_adjust(top=0.95, bottom=0.06, left=0.05, right=0.98, wspace=0.15,
							hspace=0.45)  # 调整 subplot在 fig中的边界距离
